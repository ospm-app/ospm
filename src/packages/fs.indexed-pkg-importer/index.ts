...
import fs from '../graceful-fs/index.ts';
import path from 'node:path';
import { globalInfo, globalWarn } from '../logger/index.ts';
import { packageImportMethodLogger } from '../core-loggers/index.ts';
import type {
  FilesMap,
  ImportOptions,
...
export type PackageImportMethod =
  | 'auto'
  | 'hardlink'
  | 'copy'
  | 'clone'
...
function createAutoImporter(): ImportIndexedPackage {
  ...
  function initialAuto(to: string, opts: ImportOptions): string | undefined {
    ...
    try {
      if (!hardlinkPkg(fs.linkSync, to, opts)) {
        return undefined;
      }
...
function shouldRelinkPkg(to: string, opts: ImportOptions): boolean {
  if (
    opts.disableRelinkLocalDirDeps === true &&
    opts.resolvedFrom === 'local-dir'
  ) {
    try {
...
function pkgLinkedToStore(filesMap: FilesMap, to: string): boolean {
  if (typeof filesMap['package.json'] === 'string') {
...
export function copyPkg(to: string, opts: ImportOptions): 'copy' | undefined {
  const pkgJsonPath = path.join(to, 'package.json');
